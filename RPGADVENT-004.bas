...
100 CLEAR 255: DEFINT A-Z: dim c$(20),l$(20),lv$(20),l(20,20),ob$(20),ob(20,20),obsyn$(20),obsyn(20,20),csyn$(25),csyn(25,1),act$(15),stat(25,25),statinc(25,25),looktrig(20):br$=chr$(10):stdp=1
...
155 data "lantern","lamp",1,2,1,0,0,8,0,, "glowing lantern","lantern",0,4,2,0,6,7,0,,gold,,4,3,0,0,0,0,0,, gold,,4,4,0,0,0,0,0,4,"leather cuirass","cuirass",0,2,1,0,0,0,0,,"rusty key","key",0,0,0,0,0,0,13,2, "iron gate","gate",2,99,0,0,7,4,12,2
...
173 DATA ,12,,,,,,2,1, 0,4,0,0,0,13,3,3,2, 0,0,0,0,0,12,0,0,0, 0,0,0,0,0,12,0,0,0
174 for x= 1 to 4: for y=1 to 9: read trigger(x,y):next y,x
'----------------------------------------------------------------------------------------------------------------------------
...
'special conditions
208 'if ob(8,1)<0 or ob(8,1)=lo or l(lo,6)<>1 then light=1
...
'COMMAND USE COMMAND USE COMMAND USE COMMAND USE COMMAND USE COMMAND USE COMMAND USE COMMAND USE COMMAND USE COMMAND USE COMMAND USE
        ...
        871 if ob(ob1,1)<>lo and ob(ob1,1)>-1 then ? "object not here": goto 995
        874 if ob(ob(ob1,7),1)=lo then ? "present.":triggerit=ob(ob(ob,7),8)
        875 if ob(ob1,7)<>0 then if ob(ob(ob1,7),1)<>lo and ob(ob(ob1,7),1)>-1 then print er$(6):goto 995 else triggerit=ob(ob(ob1,7),8):goto 6000
        ...
        884 if ob(ob1,6)=0 then print er$(5):goto 995
        885 if ob(ob1,6)<0 then print er$(5):goto 995 else ob2= ob(ob1,6): swap ob(ob1,1), ob(ob2,1): PRINT act$(ob(ob2,7));". ";EX$(ob(ob2,5)) ' THIS line is obsolete, need to move it to trigger now.
'------------------------------------------------------------------------------------------------------------------------------------
...
6092 if trigger(triggerit,7)>0 then triggerit=trigger(triggerit,7):goto 6000
6100 goto 9500
