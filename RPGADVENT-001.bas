...
173 DATA ,12,,,,,,2,1, 0,4,0,0,0,13,0,3,2
174 for x= 1 to 2: for y=1 to 9: read trigger(x,y):next y,x
'----------------------------------------------------------------------------------------------------------------------------
...
651 if ob(ob1,1)<>-1 then print er$(2):goto 160 else ob(ob1,1)=lo:weight=weight-ob(ob1,2): Print "Dropped." else print er$(4)
...
'COMMAND USE COMMAND USE COMMAND USE COMMAND USE COMMAND USE COMMAND USE COMMAND USE COMMAND USE COMMAND USE COMMAND USE COMMAND USE
        ...
        871 if ob(ob1,7)=0 then 872 else if ob(ob(ob1,7),1)<>lo and ob(ob(ob1,7),1)>0 then print er$(6):goto 995 else triggerit=ob(ob(ob1,7),8):goto 6000
        872 if ob(ob1,1)>-1 and ob(ob1,1)<>lo then print er$(2):goto 995
        873 if ob(ob1,6)<0 and ob(obsyn,6) <0 then print er$(5):goto 995
        874  ob2= ob(ob1,6): swap ob(ob1,1), ob(ob2,1): PRINT act$(ob(ob2,7));". ";EX$(ob(ob2,5))
'------------------------------------------------------------------------------------------------------------------------------------
...
'EXECUTE TRIGGERS
'TRIGGERS 1)swap with object, 2)insert object at loc, 3)insert object to inv, 4)trigger death,
'         5)teleport user, 6)delete object, 7)execute another trigger, 8)action message 
6000 '
...
6090 if trigger(triggerit,9)>0 then PRINT "Your stats have increased. ":goto 7300
6100 goto 9500
...
7203 print "Attack:";attack; "Defense: ";defense;"Skill: ";skill;"Stamina: ";stamina;"Luck:";luck;"Charisma: ";charisma;"Magic: ";magic
7299 goto 199
